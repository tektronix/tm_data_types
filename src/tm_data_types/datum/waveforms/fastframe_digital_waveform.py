"""FastFrame multi-frame digital waveform types and TekHSI-compatible API."""
...
from tm_data_types.datum.waveforms.fastframe_common import FastFrameMixin, FrameTimingInfo

if TYPE_CHECKING:
    from collections.abc import Iterator

__all__ = ["FastFrameDigitalWaveform", "FrameTimingInfo"]


def _apply_digital_frame_metadata(
    source: DigitalWaveform,
    dest: DigitalWaveform,
    frame_index: int,
) -> None:
    """Copy shared metadata onto a single-frame digital view."""
    ...
    dest.digital_bitmask = source.digital_bitmask
    if isinstance(source, FastFrameDigitalWaveform):
...
class FastFrameDigitalWaveform(DigitalWaveform, FastFrameMixin):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        """Initialize the FastFrame digital waveform and its multi-frame state."""
        super().__init__()
    ...
    def __iter__(self) -> Iterator[tuple[int, DigitalWaveform]]:
        """Iterate over ``(index, DigitalWaveform)`` pairs excluding the summary frame."""
    ...
    def _build_frame_view(self, index: int, *, copy: bool = False) -> DigitalWaveform:
        ...
        view = DigitalWaveform()
        _apply_digital_frame_metadata(self, view, index)
        raw_sample = np.ndarray.__new__(RawSample, shape=raw.shape, dtype=raw.dtype, buffer=raw)
        object.__setattr__(view, "y_axis_byte_values", raw_sample)
        return view

    def frame(self, index: int) -> DigitalWaveform:
        """Return a single-frame ``DigitalWaveform`` view for ``index``."""
    ...
    def frames(
        ...
        *,
        ...
        copy: bool = False,
    ) -> Iterator[tuple[int, DigitalWaveform]]:
        """Yield ``(index, DigitalWaveform)`` for each frame."""
